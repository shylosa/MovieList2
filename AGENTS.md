...
### ???? 17 ? Bugfix Mojibake comment in storage.Movie

- [internal/storage/storage.go / Movie] ???????? ??????????? ???????? ??? ?????????? `Movie` ?? ??????????? UTF-8 ????? `// Movie describes the movie record structure in the database (unified)`.

### ???? 18 ? Bugfix Mojibake comment in storage.AIResolution

- [internal/storage/storage.go / AIResolution] ???????? ??????????? ???????? ??? ?????????? `AIResolution` ?? ??????????? UTF-8 ????? `// AIResolution is the Gemini recognition cache entry (L2 Cache)`.

### ???? 19 ? Bugfix tmdb_id schema comment

- [internal/storage/storage.go / InitSchema] ???????? ??????????? inline-???????? ? ????? `movies` ?? `tmdb_id INTEGER, -- added column`.

### ???? 20 ? Bugfix lazy migration comments

- [internal/storage/storage.go / InitSchema] ???????? ??? ?????????? ????????? ????? lazy migration ?? ?????????? UTF-8 ????? ??? ????????? ??????? ?? ??????????? `duplicate column name`.

### ???? 21 ? Bugfix begin transaction comment

- [internal/storage/storage.go / SaveMoviesBatch] ???????? ??????????? ???????? ????? `BeginTx` ?? `// Begin transaction`.

### ???? 22 ? Bugfix rollback inline comment

- [internal/storage/storage.go / SaveMoviesBatch] ???????? inline-???????? ???? `defer tx.Rollback()` ?? ??????????? UTF-8 ????? ??? safe no-op ????? `Commit`.

### ???? 23 ? Bugfix prepare statement comment

- [internal/storage/storage.go / SaveMoviesBatch] ???????? ??????????? ???????? ????? `PrepareContext` ?? `// Prepare statement for performance`.

### ???? 24 ? Bugfix execute inserts comment

- [internal/storage/storage.go / SaveMoviesBatch] ???????? ??????????? ???????? ????? ?????? ??????? ?? `// Execute inserts in memory`.

### ???? 25 ? Bugfix context cancel comment

- [internal/storage/storage.go / SaveMoviesBatch] ???????? ??????????? ???????? ????????? ????? ?? `// Abort safely if context is cancelled`.

### ???? 26 ? Bugfix per-row error comment

- [internal/storage/storage.go / SaveMoviesBatch] ???????? ??????????? ???????? ? ????? ??????? ??????? ?? `// Log per-row error and continue`.

### ???? 27 ? Bugfix commit comment

- [internal/storage/storage.go / SaveMoviesBatch] ???????? ??????????? ???????? ????? `tx.Commit()` ?? `// Commit all changes at once`.

### ???? 28 ? Bugfix remaining methods comment

- [internal/storage/storage.go] ???????? ??????????? ???????? ????? `CleanMissingMovies` ?? ??????????? UTF-8 ????? ??? ????????? ????????? ??????.

### ???? 29 ? Bugfix GetMoviesByFilenames comment

- [internal/storage/storage.go / GetMoviesByFilenames] ???????? ??????????? ???????? ????????? ?? `// GetMoviesByFilenames returns a map of movies keyed by filename`.

### ???? 30 ? Encoding verification rule in .cursorrules

- [.cursorrules] ?????? ???? ?????? ????? ??????? `ENCODING` ?????????? UTF-8 ??????? `.go` ????? ???????? `python -c "open('path/to/file','rb').read().decode('utf-8')"` ? ???????????? ???? ? git ??? `UnicodeDecodeError`.
