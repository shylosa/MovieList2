...
# 🍿 MovieList 2.6.0 — Менеджер локальної медіатеки

**MovieList 2.6.0** — десктопний додаток на **Go 1.26.2 та Wails v2** для перетворення папок із відеофайлами на структурований кінокаталог. Програма автоматично збирає метадані (постери, описи, рейтинги), підтримує хмарну синхронізацію та генерує статичну HTML-вітрину для перегляду на мобільних пристроях.
...
`APP_VERSION` необов'язковий: без нього використовується 2.6.0. Значення у локальному `.env` або змінній середовища перекриває дефолт і визначає версію в заголовку програми.
