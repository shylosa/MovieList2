...
У редакторі ручна підказка працює разом із selector `Авто / Фільм / Серіал`. Стримана кнопка `Варіанти` показує до п’яти детерміновано впорядкованих TMDB-кандидатів (назва, original title, рік, тип та ID) без постерів. Першим завжди показується вже збережений TMDB-запис, щоб його можна було уточнити без переходу на сайт. Без ручної підказки пошук використовує назву файла, а не потенційно помилкову назву з БД; для транслітерату виконується обмежений fallback.
